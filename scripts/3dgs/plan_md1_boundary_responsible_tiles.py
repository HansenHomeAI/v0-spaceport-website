...
def camera_manifest_context_by_bucket(
    ...
    *,
    ...
    if not isinstance(views, list):
        return result
    for view in views:
        if not isinstance(view, Mapping) or str(view.get("bucket")) != bucket:
            continue
        image_name = Path(str(view.get("image_name") or "")).name
        if not image_name:
            continue
        result[image_name] = ordered_unique(list_strings(view.get(context_field)))
    return result
...
def candidate_context_tile_ids(strategy: Mapping[str, Any] | None) -> list[str]:
    if not strategy:
        return []
    values: list[str] = []
    for key in (
        ...
        values.extend(list_strings(strategy.get(key)))
    nested = strategy.get("quality_strategy")
    if isinstance(nested, Mapping):
        for key in (
            "context_support_tile_ids",
            ...
            values.extend(list_strings(nested.get(key)))
    return ordered_unique(values)
...
def horizon_gap(decision: Mapping[str, Any]) -> dict[str, Any] | None:
    per_bucket = decision.get("per_bucket") if isinstance(decision.get("per_bucket"), Mapping) else {}
    horizon = per_bucket.get("horizon") if isinstance(per_bucket.get("horizon"), Mapping) else {}
    deltas = horizon.get("delta") if isinstance(horizon.get("delta"), Mapping) else {}
    thresholds = decision.get("thresholds") if isinstance(decision.get("thresholds"), Mapping) else {}
    horizon_thresholds = thresholds.get("horizon") if isinstance(thresholds.get("horizon"), Mapping) else {}
...
def responsible_tiles_for_bucket(
    *,
    image_names: Sequence[str],
    support_by_image: Mapping[str, dict[str, list[str]]],
    embedded_context: Mapping[str, list[str]],
) -> tuple[Counter[str], Counter[str], dict[str, Counter[str]], list[dict[str, Any]]]:
    ...
    for image_name in image_names:
        ...
        roles_by_tile = support_by_image.get(normalized_name, {})
        support_tile_ids = sorted(roles_by_tile)
        embedded_primary = embedded_context.get(normalized_name, [])
        ...
        per_camera.append(
            {
                ...
                "embedded_context_tile_ids": embedded_primary,
    ...
    return primary_counts, support_counts, role_counts_by_tile, per_camera
...
def plan_boundary_responsible_tiles(
    *,
    ...
    horizon_images = images_by_bucket.get("horizon_camera_ids", [])
    embedded_boundary_context = camera_manifest_boundary_context(review_comparison)
    embedded_horizon_context = camera_manifest_context_by_bucket(
        review_comparison,
        bucket="horizon",
        context_field="horizon_context_tile_ids",
    )
    support_by_image = support_roles_by_image(tile_manifest)
    tile_lookup = tiles_by_id(tile_manifest)

    primary_counts, support_counts, role_counts_by_tile, per_camera = responsible_tiles_for_bucket(
        image_names=boundary_images,
        support_by_image=support_by_image,
        embedded_context=embedded_boundary_context,
    )
    horizon_primary_counts, horizon_support_counts, horizon_role_counts_by_tile, per_horizon_camera = (
        responsible_tiles_for_bucket(
            image_names=horizon_images,
            support_by_image=support_by_image,
            embedded_context=embedded_horizon_context,
        )
    )

    primary_responsible_tiles = sorted(primary_counts)
    support_tiles = sorted(support_counts)
    horizon_primary_tiles = sorted(horizon_primary_counts)
    horizon_support_tiles = sorted(horizon_support_counts)
    combined_primary_tiles = ordered_unique(primary_responsible_tiles + horizon_primary_tiles)
    combined_support_tiles = ordered_unique(support_tiles + horizon_support_tiles)
    ranked_tiles: list[dict[str, Any]] = []
    for tile_id in sorted(set(combined_primary_tiles + combined_support_tiles)):
        ...
        ranked_tiles.append(
            {
                ...
                "primary_horizon_camera_count": horizon_primary_counts.get(tile_id, 0),
                "support_horizon_camera_count": horizon_support_counts.get(tile_id, 0),
                "support_role_counts": dict(sorted(role_counts_by_tile[tile_id].items())),
                "horizon_support_role_counts": dict(sorted(horizon_role_counts_by_tile[tile_id].items())),
    ...
    ranked_tiles.sort(
        key=lambda row: (
            ...
            -int(row["primary_horizon_camera_count"]),
            -int(row["support_boundary_camera_count"]),
            -int(row["support_horizon_camera_count"]),
            str(row["tile_id"]),
        )
    )
    ...
    candidate_context_tiles = candidate_context_tile_ids(candidate_strategy)
    candidate_covered_tiles = ordered_unique(candidate_tiles + candidate_context_tiles)
    missing_candidate_targets = [reason for reason in blockers if reason not in candidate_targets]
    missing_primary_tiles = [
        tile_id
        for tile_id in primary_responsible_tiles
        if candidate_covered_tiles and tile_id not in candidate_covered_tiles
    ]
    missing_horizon_tiles = [
        tile_id
        for tile_id in horizon_primary_tiles
        if candidate_covered_tiles and tile_id not in candidate_covered_tiles
    ]
    ...
    if HORIZON_SSIM_BLOCKER in blockers and HORIZON_SSIM_BLOCKER not in candidate_targets:
        blocked_reasons.append("candidate_strategy_does_not_target_horizon_blocker")
    if missing_primary_tiles:
        blocked_reasons.append("candidate_strategy_omits_primary_boundary_tiles")
    if missing_horizon_tiles:
        blocked_reasons.append("candidate_strategy_omits_primary_horizon_tiles")
    if BOUNDARY_BLOCKER in blockers and not boundary_images:
        blocked_reasons.append("boundary_frozen_cameras_missing")
    if HORIZON_SSIM_BLOCKER in blockers and not horizon_images:
        blocked_reasons.append("horizon_frozen_cameras_missing")

    recommendation = "hold_paid_retry"
    next_step = "dry_run_tile04_boundary_horizon_strategy"
    ...
    return {
        ...
        "horizon_regression_gap": horizon_gap(decision),
        "quality_strategy_reference": quality_strategy,
        "boundary_frozen_cameras": boundary_images,
        "horizon_frozen_cameras": horizon_images,
        "per_boundary_camera": per_camera,
        "per_horizon_camera": per_horizon_camera,
        "primary_responsible_tile_ids": primary_responsible_tiles,
        "horizon_primary_responsible_tile_ids": horizon_primary_tiles,
        "combined_primary_responsible_tile_ids": combined_primary_tiles,
        "support_tile_ids": support_tiles,
        "horizon_support_tile_ids": horizon_support_tiles,
        "combined_support_tile_ids": combined_support_tiles,
        ...
        "candidate_strategy_context_tile_ids": candidate_context_tiles,
        "candidate_strategy_covered_tile_ids": candidate_covered_tiles,
        ...
        "proposed_no_spend_next_step": {
            "action": next_step,
            "tile_scope": combined_primary_tiles,
            "context_support_tile_ids": [
                tile_id for tile_id in combined_support_tiles if tile_id not in combined_primary_tiles
            ],
            "rationale": (
                "The frozen boundary and horizon cameras must be protected together; tile_04 is the horizon "
                "lever and boundary still depends on the tile_04/tile_10 overlap context."
            ),
        },
        "paid_retry_requirements_before_submit": [
            "targeted_quality_blockers must include boundary_no_required_improvement",
            "targeted_quality_blockers must include horizon_ssim_regression when horizon is currently blocked",
            "selected tiles must include every primary_responsible_tile_id",
            "or context_support_tile_ids/reuse_context_tile_ids must cover any primary tile held on rollback",
            "strategy must name the boundary frozen cameras and expected metric axis",
            "strategy must name the horizon frozen cameras and expected metric axis horizon.ssim",
