...
TILE_INPUT_HASH_ENV_KEYS = (
    ...
    "FLOATER_PRUNING_SKY_COLOR_ENABLED",
    "FLOATER_PRUNING_SKY_COLOR_MIN_SKY_VIEWS",
    "FLOATER_PRUNING_SKY_COLOR_MAX_COLOR_DISTANCE",
    "FLOATER_PRUNING_INCLUDE_PRIORITY_CONTEXT",
    "FLOATER_PRUNING_HORIZON_COVERAGE_ENABLED",
    "FLOATER_PRUNING_HORIZON_COVERAGE_MIN_PRIORITY_VIEWS",
    "FLOATER_PRUNING_HORIZON_COVERAGE_MIN_SKY_VIEWS",
    "FLOATER_PRUNING_HORIZON_COVERAGE_MIN_TOP_FRACTION",
    "FLOATER_PRUNING_HORIZON_COVERAGE_MAX_COLOR_DISTANCE",
    "GAUSSIAN_SCALE_PRUNING_ENABLED",
    "GAUSSIAN_SCALE_PRUNING_MAX_SCALE",
    "GAUSSIAN_SCALE_PRUNING_MAX_VOLUME",
...
def upload_tile_selection_manifests(tile_manifest: dict, view_buckets: dict, s3_uri: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="3dgs-tile-selection-") as tmp:
        ...
        seam_graph_path = tmpdir / "3dgs_seam_graph.json"
        ...
        if isinstance(tile_manifest.get("sfm_seam_graph"), dict):
            seam_graph_path.write_text(
                json.dumps(tile_manifest["sfm_seam_graph"], indent=2, sort_keys=True),
                encoding="utf-8",
            )
            run_command(["aws", "s3", "cp", str(seam_graph_path), f"{output_prefix}/3dgs_seam_graph.json"])
...
def build_benchmark_stages(
    *,
    ...
    for tile_id in selected_tiles:
        ...
        record_tile_budget = tile_budget_mode == "adaptive" or max_images_per_tile > 0
        stage_max_iterations = tile_max_iterations
        if record_tile_budget:
        ...
        if enable_context_density_reuse and context_density_hit is not None:
            ...
            stages.append(
                BenchmarkStage(
                    ...
                    budget_class=tile_budget.budget_class if record_tile_budget else None,
                    selected_image_count=tile_budget.selected_image_count if record_tile_budget else None,
                    max_iterations=0,
                    max_selected_images=tile_budget.max_selected_images if record_tile_budget else None,
                    input_hash=tile_budget.input_hash if record_tile_budget else None,
                    source_artifact_uri=str(context_density_hit.get("artifact_s3_uri")),
                    checkpoint_uri=None,
                    spot_enabled=False,
                    prior_duration_hours=tile_budget.prior_duration_hours if record_tile_budget else None,
                    prior_fanout_duration_hours=tile_budget.prior_fanout_duration_hours if record_tile_budget else None,
        ...
        stages.append(
            BenchmarkStage(
                ...
                budget_class=tile_budget.budget_class if record_tile_budget else None,
                selected_image_count=tile_budget.selected_image_count if record_tile_budget else None,
                max_iterations=tile_budget.max_iterations if record_tile_budget else None,
                max_selected_images=tile_budget.max_selected_images if record_tile_budget else None,
                input_hash=tile_budget.input_hash if record_tile_budget else None,
                ...
                prior_duration_hours=tile_budget.prior_duration_hours if record_tile_budget else None,
                prior_fanout_duration_hours=tile_budget.prior_fanout_duration_hours if record_tile_budget else None,
                quality_gate_status=tile_budget.quality_gate_status if record_tile_budget else None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--require-sfm-authority",
        action="store_true",
        help="Require seam_graph_sim3_v1 reducer/seam metadata before planning or submitting tiled 3DGS.",
    )
...
def main() -> int:
    ...
    reducer_metadata_s3_uri = f"{colmap_s3_uri}/reducer_metadata.json"
    seam_merge_report_s3_uri = f"{colmap_s3_uri}/seam_merge_report.json"
    ...
    tile_manifest, view_buckets, manifest_resolution = resolve_tiled_input_manifests(
        ...
        require_sfm_authority=bool(args.require_sfm_authority),
    ...
    if args.require_sfm_authority:
        training_env_overrides.setdefault("REQUIRE_SFM_AUTHORITY", "true")
    ...
    summary: dict = {
        ...
        "reducer_metadata_s3_uri": reducer_metadata_s3_uri,
        "seam_merge_report_s3_uri": seam_merge_report_s3_uri,
