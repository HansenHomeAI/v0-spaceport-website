...
while :; do
  ...
  # Nudge prompt keeps it acting without summaries. Use non-interactive exec so
  # tmux/watchdog resumes do not fail when stdin is not a TTY.
  prompt='Resume continuous development cycle per Autonomy Contract:
