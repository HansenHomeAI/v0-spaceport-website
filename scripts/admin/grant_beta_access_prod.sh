...
# Grant beta access admin permissions to an employee (PRODUCTION VERSION)
# Requirements: awscli, jq configured with access to the target AWS account/region.
# Usage:
#   ./scripts/admin/grant_beta_access_prod.sh EMAIL [--revoke]
#
...
# Fetch Auth stack outputs (PRODUCTION)
...
USER_POOL_ID=$(echo "$OUT" | jq -r '.[] | select(.OutputKey=="CognitoUserPoolId") | .OutputValue')
PERMISSIONS_TABLE="Spaceport-BetaAccessPermissions-prod"
...
if [ -z "$USER_POOL_ID" ] || [ "$USER_POOL_ID" = "null" ]; then 
  echo "CognitoUserPoolId not found in SpaceportAuthStack outputs" >&2
  exit 1
fi
