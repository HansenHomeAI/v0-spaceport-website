...
- Map bootstraps with `@googlemaps/js-api-loader` and a branch-specific `mapId` so preview builds can point at dedicated styles if needed.
...
- Flight path is drawn as a `THREE.Line` through Catmull–Rom interpolated positions generated by `overlay.latLngAltitudeToVector3` (altitude converted to meters).
...
- **2D View Only**: Ensure the referenced `mapId` has Photorealistic 3D Tiles enabled. Without it Google falls back to flat satellite imagery.
