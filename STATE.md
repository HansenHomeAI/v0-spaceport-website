...
live_compute_status: R1 strict_core Processing retry md1-r1-strict-core-1776982128-quality failed at render with gsplat CUDA OOM on ml.g4dn.xlarge; local patch now adds render_scale/cull/topk OOM fallback plus a SageMaker Training execution mode for g5.2xlarge review jobs
...
blocked_promotion_reason: fallback_tile_count remains >0 and comparative render metrics from the offline merge variants are not yet available
next_unblocked_step: commit/push the memory-bounded review renderer and training-mode launcher, wait CDK/Pages/3dgs image on the exact head, then run strict_core/support_weighted_overlap/raw_union R1 smoke reviews through SageMaker Training with render_scale=0.25 max_gaussians_per_view=500000 tile_ids=tile_02,tile_05
owner_action_needed: none
updated: 2026-04-23T17:09:16-06:00
