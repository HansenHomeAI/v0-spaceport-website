...
python3 /opt/ml/processing/input/scripts/build_md1_hybrid_artifact.py \
  ...
  --color-rgb-offset '0.008,0.12,0.0' \
  --merge-mode support_weighted_overlap \
  --label r5_teacher_context_v12_sky_v18 \
  --purpose 'Artifact-level no-training sky-preserving foreground DC color calibration from V16, selected after V17 cleared horizon PSNR but blocked on sky score.' \
