...
python3 /opt/ml/processing/input/scripts/build_md1_hybrid_artifact.py \
  ...
  --opacity-policy scale-aware \
  --scale-start-quantile 0.90 \
  --scale-full-quantile 0.99 \
  --scale-weight-power 1.0 \
  ...
  --label r5_teacher_context_0002040506_scale_v11 \
  --purpose 'Repair remaining V8 horizon PSNR/SSIM miss by applying scale-aware opacity normalization to the same teacher tile_00/tile_02/tile_04/tile_05/tile_06 replacement set while preserving the V8 tile selection.' \
