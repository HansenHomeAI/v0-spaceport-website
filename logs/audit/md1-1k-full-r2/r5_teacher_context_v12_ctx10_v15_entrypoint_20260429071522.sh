...
python3 /opt/ml/processing/input/scripts/build_md1_hybrid_artifact.py \
  ...
  --context-padding-ratio 0.10 \
  --merge-mode support_weighted_overlap \
  --label r5_teacher_context_v12_ctx10_v15 \
  --purpose 'Start from V12 best blocked repair and add 10 percent preserve-context padding including historical tile_01 to test whether a wider R5 tile_01/tile_04 horizon context closes the remaining PSNR-only miss.' \
