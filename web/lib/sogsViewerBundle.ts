...
export type SogsBundleKind = "single" | "lod-streaming";
...
function shouldProxyBundleUrl(url: URL): boolean {
  return PROXY_HOSTS.has(url.host) || isAllowedEdgeBundleUrl(url);
}
...
function buildManifestCandidates(inputUrl: URL): ManifestCandidate[] {
  ...
  if (leaf === "lod-meta.json") {
    return [
      {
        sourceManifestUrl: new URL(inputUrl.toString()),
        sourceBundleRootUrl,
  ...
  return [
    {
      sourceManifestUrl: new URL(`lod-meta.json${bundleRoot.search}`, bundleRoot),
      sourceBundleRootUrl: bundleRoot,
      rootFile: "lod-meta.json",
    },
    {
      sourceManifestUrl: new URL(`meta.json${bundleRoot.search}`, bundleRoot),
      sourceBundleRootUrl: bundleRoot,
      rootFile: "meta.json",
    },
  ];
...
function summarizeManifest(
  ...
  const isLod =
    ...
    (manifest &&
      ...
      typeof (manifest as { lodLevels?: unknown }).lodLevels === "number");
  const manifestObject = manifest && typeof manifest === "object" ? (manifest as Record<string, unknown>) : {};
  const splatCount =
    typeof manifestObject.means === "object" &&
    manifestObject.means &&
    Array.isArray((manifestObject.means as { shape?: unknown }).shape)
      ? ((manifestObject.means as { shape: number[] }).shape[0] ?? null)
      : null;
  ...
  return {
    ...
    splatCount: typeof splatCount === "number" ? splatCount : null,
...
export function resolveBundleAssetUrl(rawBundleValue: string, assetPath: string): string | null {
  ...
  return parsed ? resolveAssetUrlFromSource(new URL("./", parsed), assetPath, "direct") : null;
}

export function resolveBundleAssetUrlForTransport(
  rawBundleValue: string,
  assetPath: string,
  transport: SogsBundleTransport,
): string | null {
  const parsed = parseInputUrl(rawBundleValue);
  return parsed ? resolveAssetUrlFromSource(new URL("./", parsed), assetPath, transport) : null;
}
