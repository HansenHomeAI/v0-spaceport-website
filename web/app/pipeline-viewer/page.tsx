...
export default function PipelineViewerPage() {
  ...
  const handleLoadSfm = async () => {
    ...
    try {
      ...
      const typedBounds = bounds as SfmData["bounds"];
    ...
    <main style={pageStyles}>
      ...
      <section style={sectionGridStyles}>
        ...
        <div style={{ ...cardStyles, display: "grid", gap: "12px" }}>
          ...
          <p style={mutedTextStyles}>
            If the 3DGS output is still packaged as `model.tar.gz`, extract the `splat.ply` and provide its direct
            URL. Large files may take a while to load.
          </p>
        </div>
      </section>
          ...
          {activeTab === "sfm" && (
            <div style={overlayCardStyles}>
              <div>
                ...
                <p style={mutedTextStyles}>
                  Load COLMAP outputs and visualize the sparse point cloud plus camera frustums. Use the transform
                  dropdown to check 90-degree rotation offsets.
                </p>
              </div>
