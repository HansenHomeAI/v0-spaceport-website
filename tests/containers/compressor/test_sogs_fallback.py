...
"""
SOGS Fallback Test
Tests SOGS compression container in fallback mode (CPU-only)
"""
...
class SOGSFallbackTester:
    """Fallback tester for SOGS compression container"""
    ...
    def __init__(self, region: str = "us-west-2"):
        ...
        # Configuration - using simple tag for CPU-only container
        self.ecr_image_uri = f"{self.account_id}.dkr.ecr.{region}.amazonaws.com/spaceport-ml-sogs-compressor:simple"
        self.test_bucket = "spaceport-uploads"
        self.output_bucket = "spaceport-ml-processing"
        
        print(f"🧪 SOGS Fallback Tester initialized")
    ...
    def create_test_ply_file(self) -> str:
        ...
        ply_content = """ply
format ascii 1.0
comment Generated test model for SOGS compression testing (fallback mode)
element vertex 1000
property float x
property float y
property float z
...
"""
        ...
        vertices = []
        for i in range(1000):
            # Position (cube distribution for simplicity)
            x = random.uniform(-5, 5)
            y = random.uniform(-5, 5)
            z = random.uniform(-5, 5)
            
            # Normal (random normalized)
            ...
            # Scale (reasonable sizes)
            s0 = random.uniform(0.1, 1.0)
            s1 = random.uniform(0.1, 1.0)
            s2 = random.uniform(0.1, 1.0)
        ...
        # Upload to S3
        s3_key = f"test-data/fallback-test-{int(time.time())}.ply"
    ...
    def run_compression_job(self, input_s3_uri: str) -> Dict:
        ...
        job_name = f"sogs-fallback-test-{int(time.time())}"
        ...
        job_config = {
            ...
            'ProcessingResources': {
                'ClusterConfig': {
                    'InstanceCount': 1,
                    'InstanceType': 'ml.c6i.2xlarge',  # Smaller instance for CPU-only
                    'VolumeSizeInGB': 30
                }
            },
            'AppSpecification': {
            ...
            'Environment': {
                'LOG_LEVEL': 'INFO',
                'PYTHONUNBUFFERED': '1',
                'SOGS_FALLBACK_MODE': 'true'  # Force fallback mode
            }
        }
            ...
            if report_key:
                ...
                try:
                    ...
                    if 'compression_metrics' in report_data:
                        ...
                        # Check if compression was effective (lower threshold for fallback)
                        compression_ratio = metrics.get('compression_ratio', 0)
                        if compression_ratio >= 3.0:  # At least 3:1 ratio for fallback
                            print("   ✅ Compression ratio acceptable for fallback mode (≥3:1)")
    ...
    def run_full_test(self) -> Dict:
        ...
        print("🎯 Starting SOGS Fallback Test (CPU-only)")
        print("=" * 50)
        ...
        try:
            ...
            if analysis_result.get('status') == 'success':
                if analysis_result.get('compression_effective', False):
                    ...
                    print("\n🎉 SOGS Fallback Test PASSED!")
                else:
                    test_results['overall_status'] = 'partial'
                    print("\n⚠️  SOGS Fallback Test PARTIAL - compression below expectations")
            else:
                test_results['overall_status'] = 'failed'
                test_results['failure_reason'] = 'Result analysis failed'
                print("\n❌ SOGS Fallback Test FAILED")
...
def main():
    """Main test function"""
    parser = argparse.ArgumentParser(description='SOGS Fallback Test')
    ...
    # Run the test
    tester = SOGSFallbackTester(region=args.region)
    ...
    if results['overall_status'] == 'success':
        print("\n✅ SOGS compression container is working correctly in fallback mode!")
        print("💡 For better performance, deploy the CUDA-enabled version later.")
        return 0
    else:
