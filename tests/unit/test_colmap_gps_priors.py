...
class ColmapGpsPriorTests(unittest.TestCase):
    ...
    def test_visibility_cell_jurisdiction_rejects_far_owner_points(self):
        with tempfile.TemporaryDirectory() as tmp, mock.patch.dict(
            ...
            owner_bounds = pipeline.chunk_jurisdictions[owner_cell]["jurisdiction_bounds"]
            far_cell = max(
                pipeline.chunk_jurisdictions,
                key=lambda index: abs(
                    pipeline.chunk_jurisdictions[index]["bounds"]["min_x"] - owner_bounds["min_x"]
                )
                + abs(pipeline.chunk_jurisdictions[index]["bounds"]["min_y"] - owner_bounds["min_y"]),
            )
            far_bounds = pipeline.chunk_jurisdictions[far_cell]["bounds"]
            inside_point = [
                "1",
                str((owner_bounds["min_x"] + owner_bounds["max_x"]) / 2.0),
                str((owner_bounds["min_y"] + owner_bounds["max_y"]) / 2.0),
                "0",
                "255",
                "255",
                "255",
            ...
            inside_ok, inside_reason = pipeline.point_passes_visibility_jurisdiction(
                inside_point,
                image_id_to_name={1: owner_name},
            )
            far_ok, far_reason = pipeline.point_passes_visibility_jurisdiction(
                far_point,
                image_id_to_name={1: owner_name},
            )

            self.assertTrue(inside_ok)
            self.assertEqual(inside_reason, "inside_owner_jurisdiction")
    ...
    def test_prepare_chunk_database_falls_back_to_subset_clone_when_backup_hits_disk_full(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.database_path = root / "database.db"
            ...
            real_retry = pipeline.run_sqlite_operation_with_retry
            clone_attempts = 0

            def flaky_retry(stage, operation):
                nonlocal clone_attempts
                if stage.startswith("clone_database_for_chunk:"):
                    clone_attempts += 1
                    if clone_attempts == 1:
                        raise sqlite3.OperationalError("database or disk is full")
                return real_retry(stage, operation)

            with mock.patch.object(pipeline, "run_sqlite_operation_with_retry", side_effect=flaky_retry):
                chunk_db = pipeline.prepare_chunk_database(chunk)
            ...
            self.assertEqual(clone_attempts, 1)
            self.assertEqual(image_names, ["IMG_01.jpg", "IMG_02.jpg"])
            self.assertEqual(camera_ids, [1])
            self.assertEqual(keypoint_ids, [1, 2])
            self.assertEqual(descriptor_ids, [1, 2])
    ...
    def test_prepare_global_database_for_chunking_normalizes_once_without_force(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.database_path = root / "database.db"
            ...
            with mock.patch.object(
                pipeline,
                "normalize_sqlite_database_for_chunking",
                wraps=pipeline.normalize_sqlite_database_for_chunking,
            ) as normalize_mock:
                pipeline.prepare_global_database_for_chunking()
                pipeline.prepare_global_database_for_chunking()
                pipeline.prepare_global_database_for_chunking(force=True)

            self.assertEqual(normalize_mock.call_count, 2)

    def test_prepare_chunk_database_rebuilds_clone_after_corruption_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.database_path = root / "database.db"
    ...
    def test_merge_chunk_models_inserts_component_bridge_connector_and_continues(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.chunk_planner = "footprint_graph_v1"
            ...
            third_text_dir = root / "text_02"
            merged_first_text_dir = root / "merged_first_text"
            merged_second_text_dir = root / "merged_second_text"
            merged_final_text_dir = root / "merged_final_text"
            connector_text_dir = root / "connector_text"
            for directory in (
                first_model_dir,
                second_model_dir,
                third_model_dir,
            ...
            (connector_text_dir / "images.txt").write_text(
                "10 1 0 0 0 0 0 0 1 IMG_02.jpg\n0 0 -1\n11 1 0 0 0 0 0 0 1 IMG_03.jpg\n0 0 -1\n",
                encoding="utf-8",
            )
            (merged_second_text_dir / "images.txt").write_text(
                "12 1 0 0 0 0 0 0 1 IMG_02.jpg\n0 0 -1\n13 1 0 0 0 0 0 0 1 IMG_03.jpg\n0 0 -1\n14 1 0 0 0 0 0 0 1 IMG_04.jpg\n0 0 -1\n15 1 0 0 0 0 0 0 1 IMG_05.jpg\n0 0 -1\n",
                encoding="utf-8",
            )
            (merged_final_text_dir / "images.txt").write_text(
                "16 1 0 0 0 0 0 0 1 IMG_01.jpg\n0 0 -1\n17 1 0 0 0 0 0 0 1 IMG_02.jpg\n0 0 -1\n18 1 0 0 0 0 0 0 1 IMG_03.jpg\n0 0 -1\n19 1 0 0 0 0 0 0 1 IMG_04.jpg\n0 0 -1\n20 1 0 0 0 0 0 0 1 IMG_05.jpg\n0 0 -1\n",
                encoding="utf-8",
            )
            chunk_models = [
                run_colmap_sfm.ModelSummary(
                ...
                run_colmap_sfm.ModelSummary(
                    ...
                    points_3d=1000,
                    binary_dir=second_model_dir,
                    image_names=["IMG_03.jpg", "IMG_04.jpg"],
                    source_chunk_indexes=[1],
                ),
                run_colmap_sfm.ModelSummary(
                    stage="chunk_02_mapper_initial",
            ...
            connector_model = run_colmap_sfm.ModelSummary(
                stage="chunk_model_component_bridge_02_00_02_seed_01_point_triangulator_01",
                text_dir=connector_text_dir,
                cameras_registered=1,
                images_registered=2,
                points_3d=800,
                binary_dir=root / "connector_bin",
                image_names=["IMG_01.jpg", "IMG_02.jpg", "IMG_03.jpg", "IMG_04.jpg", "IMG_05.jpg"],
            ...
            adjusted_model = run_colmap_sfm.ModelSummary(
                ...
                images_registered=5,
                points_3d=2300,
                binary_dir=pipeline.work_dir / "merged_chunk_model_03_attempt_01",
                image_names=["IMG_01.jpg", "IMG_02.jpg", "IMG_03.jpg", "IMG_04.jpg", "IMG_05.jpg"],
                source_chunk_indexes=[0, 1, 2],
            )
            recovery_record = {
                "status": "connector_inserted",
                "inserted_connector_stage": connector_model.stage,
            }
            ...
            with mock.patch.object(run_colmap_sfm, "stream_command") as stream_command_mock, mock.patch.object(
                ...
                return_value=(connector_model, recovery_record),
            ) as recovery_mock, mock.patch.object(
                pipeline,
                "run_bundle_adjuster",
                return_value=adjusted_model,
            ):
            ...
            merger_commands = [call.args[0] for call in stream_command_mock.call_args_list]
            self.assertEqual(len(merger_commands), 3)
            self.assertEqual(pipeline.chunk_merge_proof["merge_component_recovery_records"], [recovery_record])
            self.assertEqual(pipeline.chunk_merge_summary["merge_component_recovery_records"], [recovery_record])

    def test_attempt_pending_merge_bridge_recovery_uses_full_component_scope(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.chunk_planner = "footprint_graph_v1"
            ...
            pending_models = [
                ...
                run_colmap_sfm.ModelSummary(
                    ...
                    image_names=["IMG_05.jpg", "IMG_06.jpg", "IMG_07.jpg"],
                    source_chunk_indexes=[2],
                ),
            ]
            connector_model = run_colmap_sfm.ModelSummary(
                stage="chunk_model_component_bridge_04_00_02_seed_01_point_triangulator_01",
                text_dir=root / "connector_text",
                cameras_registered=1,
                images_registered=5,
                points_3d=200,
                binary_dir=root / "connector_bin",
                image_names=["IMG_00.jpg", "IMG_01.jpg", "IMG_02.jpg", "IMG_03.jpg", "IMG_04.jpg", "IMG_05.jpg", "IMG_06.jpg", "IMG_07.jpg"],
                source_chunk_indexes=[0, 1, 2],
            )
            registered_names_by_stage = {
                "chunk_00_mapper_initial": {"IMG_00.jpg", "IMG_01.jpg", "IMG_02.jpg"},
                "chunk_01_mapper_initial": {"IMG_03.jpg", "IMG_04.jpg", "IMG_05.jpg"},
                "chunk_02_mapper_initial": {"IMG_05.jpg", "IMG_06.jpg", "IMG_07.jpg"},
                "chunk_model_component_bridge_04_00_02_seed_01_point_triangulator_01": {
                    "IMG_02.jpg",
                    "IMG_03.jpg",
                    "IMG_04.jpg",
                    "IMG_05.jpg",
            ...
            with mock.patch.object(
                ...
                return_value=(connector_model, ["IMG_02.jpg", "IMG_03.jpg"], False, "frontier_only"),
            ) as seam_mock:
            ...
            self.assertEqual(
                ...
                ["IMG_00.jpg", "IMG_01.jpg", "IMG_02.jpg", "IMG_03.jpg", "IMG_04.jpg", "IMG_05.jpg", "IMG_06.jpg", "IMG_07.jpg"],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                seam_mock.call_args.kwargs["stage_prefix"],
    ...
    def test_merge_chunk_models_records_component_failure_summary(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            recovery_record = {
                "status": "no_candidate",
                "pending_component_count": 2,
            }

            with mock.patch.object(
                pipeline,
                "attempt_pending_merge_bridge_recovery",
                return_value=(None, recovery_record),
            ):
                with self.assertRaisesRegex(
                    RuntimeError,
                    "Failed to merge chunk models: no overlapping registered images produced a usable merge",
            ...
            self.assertEqual(pipeline.chunk_merge_summary["pending_component_count"], 2)
            self.assertEqual(
                pipeline.chunk_merge_proof["merge_component_recovery_records"],
                [recovery_record],
            )
            self.assertEqual(
                pipeline.chunk_merge_proof["pending_stages"],
                ["chunk_00_mapper_initial", "chunk_01_mapper_initial"],
            )
    ...
    def test_run_parent_seam_registration_includes_seed_registered_names_in_frontier(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                ...
                pipeline.run_parent_seam_registration(
                    seed_model=seed_model,
                    chunk_plan=chunk_plan,
                    stage_prefix="parent_seam",
                    run_final_bundle_adjustment=False,
                    frontier_names=["IMG_04.jpg"],
                )
    ...
    def test_run_parent_seam_registration_preserves_original_seed_artifacts_when_reused(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.parent_seam_registration_cycles = 1
            seed_text_dir = root / "seed_text"
            seed_binary_dir = root / "seed_bin"
            seed_text_dir.mkdir(parents=True, exist_ok=True)
            seed_binary_dir.mkdir(parents=True, exist_ok=True)
            (seed_text_dir / "images.txt").write_text(
                "\n".join(
                    [
                        "1 1 0 0 0 0 0 0 1 A.jpg",
            ...
            with mock.patch.object(
                ...
                result = pipeline.run_parent_seam_registration(
                    seed_model=seed_model,
                    chunk_plan=chunk_plan,
                    stage_prefix="chunk_model_seam_09",
                    run_final_bundle_adjustment=False,
                )
    ...
    def test_repair_disconnected_chunk_model_components_uses_seam_registration_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIs(repaired_models[-1], bridge_model)
            self.assertEqual(seam_mock.call_count, 1)
            self.assertEqual(seam_mock.call_args.kwargs["stage_prefix"], "chunk_01_02_merge_bridge_seed_01")
            self.assertFalse(seam_mock.call_args.kwargs["run_final_bundle_adjustment"])
            run_chunk_mock.assert_not_called()

    def test_repair_disconnected_chunk_model_components_retries_second_bridge_seed(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.chunk_planner = "footprint_graph_v1"
            ...
            bridge_model = run_colmap_sfm.ModelSummary(
                stage="chunk_01_02_merge_bridge_seed_02_point_triangulator_01",
                text_dir=root / "bridge_text",
                cameras_registered=1,
                images_registered=5,
                points_3d=200,
            ...
            registered_names_by_stage = {
                ...
                "chunk_02_mapper_initial": {"IMG_04.jpg", "IMG_05.jpg", "IMG_06.jpg"},
                "chunk_03_mapper_initial": {"IMG_05.jpg", "IMG_06.jpg", "IMG_07.jpg"},
                "chunk_01_02_merge_bridge_seed_02_point_triangulator_01": {
                    "IMG_02.jpg",
                    "IMG_03.jpg",
                    "IMG_04.jpg",
                    "IMG_05.jpg",
            ...
            with mock.patch.object(
                ...
                pipeline,
                "run_parent_seam_registration_with_retry",
                side_effect=[RuntimeError("first seed failed"), (bridge_model, ["IMG_03.jpg"], False, "frontier_only")],
            ) as seam_mock, mock.patch.object(
                pipeline,
                "run_chunk_pipeline",
            ) as run_chunk_mock:
            ...
            self.assertIs(repaired_models[-1], bridge_model)
            self.assertEqual(seam_mock.call_count, 2)
            self.assertFalse(seam_mock.call_args_list[0].kwargs["run_final_bundle_adjustment"])
            self.assertFalse(seam_mock.call_args_list[1].kwargs["run_final_bundle_adjustment"])
            run_chunk_mock.assert_not_called()

    def test_repair_disconnected_chunk_model_components_keeps_original_models_when_all_bridge_seeds_fail(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            pipeline = run_colmap_sfm.ColmapPipeline(root / "input", root / "output")
            pipeline.chunk_planner = "footprint_graph_v1"
            ...
            with mock.patch.object(
                ...
                pipeline,
                "run_parent_seam_registration_with_retry",
                side_effect=[RuntimeError("first seed failed"), RuntimeError("second seed failed")],
            ):
                repaired_plans, repaired_models = pipeline.repair_disconnected_chunk_model_components(
                    chunk_plans=chunk_plans,
                    chunk_models=chunk_models,
            ...
            self.assertEqual(repaired_plans, chunk_plans)
