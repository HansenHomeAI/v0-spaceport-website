...
class AuthStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, env_config: dict, **kwargs) -> None:
        ...
        # ========== MODEL DELIVERY ADMIN SYSTEM ==========
        model_delivery_lambda_role = iam.Role(
            self, "ModelDeliveryLambdaRole",
            assumed_by=iam.ServicePrincipal("lambda.amazonaws.com"),
            managed_policies=[
                iam.ManagedPolicy.from_aws_managed_policy_name("service-role/AWSLambdaBasicExecutionRole")
            ],
            inline_policies={
                "ModelDeliveryPolicy": iam.PolicyDocument(
                    statements=[
                        iam.PolicyStatement(
                            effect=iam.Effect.ALLOW,
                            actions=[
        ...
        model_delivery_lambda = lambda_.Function(
            ...
            environment={
                ...
                "PERMISSIONS_TABLE_NAME": beta_access_permissions_table.table_name,
                "RESEND_API_KEY": os.environ.get("RESEND_API_KEY", ""),
                "APP_BASE_URL": os.environ.get("APP_BASE_URL", "https://spcprt.com"),
            },
        )

        projects_table.grant_read_write_data(model_delivery_lambda)
        beta_access_permissions_table.grant_read_data(model_delivery_lambda)
        ...
        model_delivery_api = apigw.RestApi(
            ...
            description="Administrative endpoints for delivering finished 3D models",
            default_cors_preflight_options=apigw.CorsOptions(
                allow_origins=apigw.Cors.ALL_ORIGINS,
                allow_methods=apigw.Cors.ALL_METHODS,
                allow_headers=[
        ...
        delivery_admin_resource = model_delivery_api.root.add_resource("admin")
        model_delivery_resource = delivery_admin_resource.add_resource("model-delivery")

        delivery_authorizer = apigw.CognitoUserPoolsAuthorizer(
            self,
            "ModelDeliveryAuthorizer",
            cognito_user_pools=[user_pool],
        )

        check_permission_resource = model_delivery_resource.add_resource("check-permission")
        check_permission_resource.add_method(
            "GET",
        ...
        list_projects_resource = model_delivery_resource.add_resource("list-projects")
        list_projects_resource.add_method(
            "POST",
            apigw.LambdaIntegration(model_delivery_lambda),
            authorization_type=apigw.AuthorizationType.COGNITO,
            authorizer=delivery_authorizer,
        ...
        send_delivery_resource = model_delivery_resource.add_resource("send")
        send_delivery_resource.add_method(
            "POST",
            apigw.LambdaIntegration(model_delivery_lambda),
            authorization_type=apigw.AuthorizationType.COGNITO,
            authorizer=delivery_authorizer,
        ...
        CfnOutput(self, "ModelDeliveryAdminLambdaArn", value=model_delivery_lambda.function_arn)
