...
class MLPipelineStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, env_config: dict, **kwargs) -> None:
        ...
        # Branch previews reuse the shared staging upload bucket so ML-only
        # previews can deploy without creating additional API Gateway-backed
        # Spaceport stacks.
        ...
        # This bucket is owned by the main/shared Spaceport stack, we just reference it.
        ...
        sfm_image_tag = get_ecr_branch_suffix(self.branch_name) or "latest"
        ...
        start_job_lambda = lambda_.Function(
            ...
            environment={
                ...
                # Branch previews pin the SfM image tag so container swaps stay isolated.
                "SFM_ECR_TAG": sfm_image_tag,
            }
        )
