...
def lambda_handler(event, context):
    ...
    try:
        ...
        sfm_tag = os.environ.get('SFM_ECR_TAG', 'latest')
        gaussian_tag = os.environ.get('GAUSSIAN_ECR_TAG', 'latest')
        compressor_tag = os.environ.get('COMPRESSOR_ECR_TAG', 'latest')
        ...
        def resolve_ecr_uri(repo_name, fallback_repo_name, image_tag):
            ...
            try:
                ...
                if error_code == 'RepositoryNotFoundException':
                    ...
                    try:
                        ...
                        return f"{account_id}.dkr.ecr.{region}.amazonaws.com/{fallback_repo_name}:{image_tag}"
                    except Exception as e2:
                        # If fallback also doesn't exist, use it anyway (will fail at runtime with clear error)
                        print(f"Warning: Neither {repo_name} nor {fallback_repo_name} found, using fallback tag {image_tag}")
                        return f"{account_id}.dkr.ecr.{region}.amazonaws.com/{fallback_repo_name}:{image_tag}"
                else:
        ...
        sfm_image_uri = resolve_ecr_uri(sfm_repo, sfm_repo_fallback, sfm_tag)
        gaussian_image_uri = resolve_ecr_uri(gaussian_repo, gaussian_repo_fallback, gaussian_tag)
        compressor_image_uri = resolve_ecr_uri(compressor_repo, compressor_repo_fallback, compressor_tag)
