...
def extract_edge_lod_bundle_url(execution_response):
    output = execution_response.get('output')
    if not output:
        return None
    ...
    publish_result = parsed.get('publishResult') or {}
    if isinstance(publish_result, dict):
        edge_bundle_url = publish_result.get('edgeLodBundleUrl')
        if isinstance(edge_bundle_url, str) and edge_bundle_url.strip():
            return edge_bundle_url.strip()
    ...
    if isinstance(notification_result, dict):
        payload = notification_result.get('Payload') or {}
        if isinstance(payload, dict):
            edge_bundle_url = payload.get('edgeLodBundleUrl')
            if isinstance(edge_bundle_url, str) and edge_bundle_url.strip():
                return edge_bundle_url.strip()
...
def lambda_handler(event, context):
    ...
    try:
        ...
        edge_lod_bundle_url = extract_edge_lod_bundle_url(response) if status == 'SUCCEEDED' else None
        ...
        return {
            ...
            'body': json.dumps({
                ...
                **({'edgeLodBundleUrl': edge_lod_bundle_url} if edge_lod_bundle_url else {})
            })
        }
