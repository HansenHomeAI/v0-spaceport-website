...
def lambda_handler(event, context):
    ...
    - /api/spin-path/optimize: Optimize real-path spin exports from overlap settings
    - /api/spin-path/export/battery/{id}: Download staged real-path spin CSVs
    ...
    try:
        ...
        elif resource_path == '/api/spin-path/optimize':
            return handle_spin_path_optimize(designer, body, cors_headers)
        elif resource_path == '/api/spin-path/export/battery/{id}':
            battery_id = path_parameters.get('id')
            return handle_spin_path_export_battery(designer, body, battery_id, cors_headers)
...
def _parse_real_path_overlap_config(raw_overlap_config: Optional[Dict]) -> Dict:
    if not isinstance(raw_overlap_config, dict):
    ...
    if capture_interval_seconds is None or capture_interval_seconds <= 0:
        capture_interval_seconds = capture_spacing_ft / speed_fps
    if yaw_rate_deg_per_sec is None or yaw_rate_deg_per_sec < 0:
        raise ValueError("overlapConfig.yawRateDegPerSec must be 0 or greater")
    if capture_arc_deg is None or capture_arc_deg < 0:
    ...
    pitch_sequence = []
    if isinstance(raw_pitch_sequence, list):
        for value in raw_pitch_sequence:
            numeric_value = _parse_optional_float(value, None)
            if numeric_value is None:
                continue
            pitch_sequence.append(-abs(numeric_value))
...
def _pick_progress_value(values: List[float], total_count: int, index: int, fallback: float) -> float:
    if not values:
        return fallback
    if len(values) == 1 or total_count <= 1:
        return values[0]

    scaled_index = (index * (len(values) - 1)) / max(total_count - 1, 1)
    lower_index = int(math.floor(scaled_index))
    upper_index = min(len(values) - 1, int(math.ceil(scaled_index)))
    fraction = scaled_index - lower_index
    return values[lower_index] + ((values[upper_index] - values[lower_index]) * fraction)


def _append_local_rendered_point(points: List[Dict], point: Dict) -> None:
    previous = points[-1] if points else None
    if previous and (
        abs(previous['x'] - point['x']) < 1e-6
        and abs(previous['y'] - point['y']) < 1e-6
        and abs(previous['altitude'] - point['altitude']) < 1e-6
    ):
        return
    points.append(point)


def _append_linear_rendered_segment(points: List[Dict], start: Dict, end: Dict) -> None:
    distance_ft = math.hypot(end['x'] - start['x'], end['y'] - start['y'])
    steps = max(2, int(math.ceil(distance_ft / 60.0)))
    for step_index in range(1, steps + 1):
        progress = step_index / steps
        _append_local_rendered_point(points, {
...
def _append_bezier_rendered_segment(points: List[Dict], start: Dict, control_start: Dict, control_end: Dict, end: Dict) -> None:
    chord_length_ft = math.hypot(end['x'] - start['x'], end['y'] - start['y'])
    steps = max(8, int(math.ceil(chord_length_ft / 25.0)))
    for step_index in range(1, steps + 1):
        t = step_index / steps
        one_minus_t = 1.0 - t
        _append_local_rendered_point(points, {
            'x': (one_minus_t ** 3 * start['x'])
                 + (3 * one_minus_t ** 2 * t * control_start['x'])
                 + (3 * one_minus_t * t ** 2 * control_end['x'])
                 + (t ** 3 * end['x']),
            'y': (one_minus_t ** 3 * start['y'])
                 + (3 * one_minus_t ** 2 * t * control_start['y'])
                 + (3 * one_minus_t * t ** 2 * control_end['y'])
                 + (t ** 3 * end['y']),
            'altitude': start['altitude'] + ((end['altitude'] - start['altitude']) * t),
        })


def _build_rendered_local_path_points(waypoint_records: List[Dict], designer: SpiralDesigner) -> List[Dict]:
    if not waypoint_records:
        return []
    if len(waypoint_records) == 1:
        first = waypoint_records[0]
        return [{'x': first['x'], 'y': first['y'], 'altitude': first['altitude']}]

    local_waypoints = [{
        'x': float(record['x']),
        'y': float(record['y']),
        'altitude': float(record['altitude']),
        'curve_ft': max(0.0, float(record.get('curve_size_meters', 0.0)) / designer.FT2M),
    } for record in waypoint_records]

    rendered_points: List[Dict] = []
    last_point = {
        'x': local_waypoints[0]['x'],
        'y': local_waypoints[0]['y'],
        'altitude': local_waypoints[0]['altitude'],
    }
    _append_local_rendered_point(rendered_points, dict(last_point))

    for waypoint_index in range(1, len(local_waypoints) - 1):
        previous_waypoint = local_waypoints[waypoint_index - 1]
        current_waypoint = local_waypoints[waypoint_index]
        next_waypoint = local_waypoints[waypoint_index + 1]

        incoming_dx = current_waypoint['x'] - previous_waypoint['x']
        incoming_dy = current_waypoint['y'] - previous_waypoint['y']
        outgoing_dx = next_waypoint['x'] - current_waypoint['x']
        outgoing_dy = next_waypoint['y'] - current_waypoint['y']
        incoming_length = math.hypot(incoming_dx, incoming_dy)
        outgoing_length = math.hypot(outgoing_dx, outgoing_dy)

        if incoming_length < 1e-3 or outgoing_length < 1e-3:
            _append_linear_rendered_segment(rendered_points, last_point, current_waypoint)
            last_point = {
                'x': current_waypoint['x'],
                'y': current_waypoint['y'],
                'altitude': current_waypoint['altitude'],
            }
            continue

        incoming_unit_x = incoming_dx / incoming_length
        incoming_unit_y = incoming_dy / incoming_length
        outgoing_unit_x = outgoing_dx / outgoing_length
        outgoing_unit_y = outgoing_dy / outgoing_length

        clamped_cosine = max(
            -1.0,
            min(1.0, (incoming_unit_x * outgoing_unit_x) + (incoming_unit_y * outgoing_unit_y)),
        )
        turn_angle = math.acos(clamped_cosine)
        if (not math.isfinite(turn_angle)) or turn_angle < 1e-3 or abs(math.pi - turn_angle) < 1e-3:
            _append_linear_rendered_segment(rendered_points, last_point, current_waypoint)
            last_point = {
                'x': current_waypoint['x'],
                'y': current_waypoint['y'],
        ...
        requested_lead_distance_ft = max(0.0, current_waypoint['curve_ft'])
        tangent_scale = math.tan(turn_angle / 2.0)
        if requested_lead_distance_ft < 1e-3 or tangent_scale < 1e-6:
            _append_linear_rendered_segment(rendered_points, last_point, current_waypoint)
            last_point = {
                'x': current_waypoint['x'],
                'y': current_waypoint['y'],
        ...
        max_tangent_distance = min(incoming_length, outgoing_length) * 0.49
        tangent_distance = min(requested_lead_distance_ft, max_tangent_distance)
        if tangent_distance < 1e-3:
            _append_linear_rendered_segment(rendered_points, last_point, current_waypoint)
            last_point = {
                'x': current_waypoint['x'],
                'y': current_waypoint['y'],
        ...
        tangent_start = {
            'x': current_waypoint['x'] - (incoming_unit_x * tangent_distance),
            'y': current_waypoint['y'] - (incoming_unit_y * tangent_distance),
            'altitude': current_waypoint['altitude']
                        - ((current_waypoint['altitude'] - previous_waypoint['altitude']) * (tangent_distance / incoming_length)),
        }
        tangent_end = {
            'x': current_waypoint['x'] + (outgoing_unit_x * tangent_distance),
            'y': current_waypoint['y'] + (outgoing_unit_y * tangent_distance),
            'altitude': current_waypoint['altitude']
                        + ((next_waypoint['altitude'] - current_waypoint['altitude']) * (tangent_distance / outgoing_length)),
        }

        _append_linear_rendered_segment(rendered_points, last_point, tangent_start)

        effective_radius_ft = tangent_distance / tangent_scale
        handle_distance_ft = (4.0 / 3.0) * math.tan(turn_angle / 4.0) * effective_radius_ft
        control_start = {
            'x': tangent_start['x'] + (incoming_unit_x * handle_distance_ft),
            'y': tangent_start['y'] + (incoming_unit_y * handle_distance_ft),
            'altitude': tangent_start['altitude'],
        }
        control_end = {
            'x': tangent_end['x'] - (outgoing_unit_x * handle_distance_ft),
            'y': tangent_end['y'] - (outgoing_unit_y * handle_distance_ft),
            'altitude': tangent_end['altitude'],
        }

        _append_bezier_rendered_segment(rendered_points, tangent_start, control_start, control_end, tangent_end)
        last_point = dict(tangent_end)

    _append_linear_rendered_segment(rendered_points, last_point, local_waypoints[-1])
    return rendered_points


def _build_cumulative_distances(rendered_points: List[Dict]) -> List[float]:
    cumulative_distances = [0.0]
    for index in range(1, len(rendered_points)):
        cumulative_distances.append(
            cumulative_distances[-1]
            + math.hypot(
                rendered_points[index]['x'] - rendered_points[index - 1]['x'],
                rendered_points[index]['y'] - rendered_points[index - 1]['y'],
            )
        )
    return cumulative_distances


def _interpolate_rendered_point(rendered_points: List[Dict], cumulative_distances: List[float], target_distance_ft: float) -> Dict:
    if not rendered_points:
        return {'x': 0.0, 'y': 0.0, 'altitude': 0.0, 'distance_ft': 0.0}
    if target_distance_ft <= 0.0:
        first = rendered_points[0]
        return {'x': first['x'], 'y': first['y'], 'altitude': first['altitude'], 'distance_ft': 0.0}
    if target_distance_ft >= cumulative_distances[-1]:
        last = rendered_points[-1]
        return {
            'x': last['x'],
            'y': last['y'],
            'altitude': last['altitude'],
            'distance_ft': cumulative_distances[-1],
        }

    segment_index = 1
    while segment_index < len(cumulative_distances) and cumulative_distances[segment_index] < target_distance_ft:
        segment_index += 1

    upper_index = min(len(rendered_points) - 1, segment_index)
    lower_index = max(0, upper_index - 1)
    lower_distance = cumulative_distances[lower_index]
    upper_distance = cumulative_distances[upper_index]
    span = max(upper_distance - lower_distance, 1e-9)
    fraction = (target_distance_ft - lower_distance) / span
    lower_point = rendered_points[lower_index]
    upper_point = rendered_points[upper_index]
    return {
        'x': lower_point['x'] + ((upper_point['x'] - lower_point['x']) * fraction),
        'y': lower_point['y'] + ((upper_point['y'] - lower_point['y']) * fraction),
        'altitude': lower_point['altitude'] + ((upper_point['altitude'] - lower_point['altitude']) * fraction),
        'distance_ft': target_distance_ft,
    }


def _resample_rendered_points(rendered_points: List[Dict], spacing_ft: float) -> Tuple[List[Dict], float]:
    if not rendered_points:
        return [], 0.0

    cumulative_distances = _build_cumulative_distances(rendered_points)
    total_distance_ft = cumulative_distances[-1]
    if total_distance_ft <= 1e-6:
        first = rendered_points[0]
        return ([{
            'x': first['x'],
            'y': first['y'],
    ...
    sample_spacing_ft = max(5.0, float(spacing_ft))
    sampled_points = [_interpolate_rendered_point(rendered_points, cumulative_distances, 0.0)]
    target_distance_ft = sample_spacing_ft
    while target_distance_ft < total_distance_ft:
        sampled_points.append(_interpolate_rendered_point(rendered_points, cumulative_distances, target_distance_ft))
        target_distance_ft += sample_spacing_ft

    if sampled_points[-1]['distance_ft'] < total_distance_ft - 1e-6:
        sampled_points.append(_interpolate_rendered_point(rendered_points, cumulative_distances, total_distance_ft))
...
def _assign_export_curve_feet(sampled_points: List[Dict], index: int) -> float:
    ...
    previous_distance = math.hypot(
        sampled_points[index]['x'] - sampled_points[index - 1]['x'],
        sampled_points[index]['y'] - sampled_points[index - 1]['y'],
    )
    next_distance = math.hypot(
        sampled_points[index + 1]['x'] - sampled_points[index]['x'],
        sampled_points[index + 1]['y'] - sampled_points[index]['y'],
    )
...
def _build_real_path_row_data(designer: SpiralDesigner, sampled_points: List[Dict], center: Dict, overlap_config: Dict) -> Tuple[List[Dict], List[Dict]]:
    ...
    for index, point in enumerate(sampled_points):
        ...
        pitch_deg = _pick_progress_value(
            ...
            total_count,
            index,
            overlap_config['default_pitch_deg'],
        )
...
def handle_spin_path_optimize(designer, body, cors_headers):
    try:
        battery_minutes = float(body.get('batteryMinutes', 20))
        batteries = int(body.get('batteries', 1))
        ...
        overlap_config = _parse_real_path_overlap_config(body.get('overlapConfig'))
        ...
        params = {
            'slices': batteries,
            'N': int(optimized_params['N']),
            'r0': float(optimized_params['r0']),
            'rHold': float(optimized_params['rHold']),
        }
        actual_min = optimized_params.get('actualMinExpansionDist')
        actual_max = optimized_params.get('actualMaxExpansionDist')
        if actual_min is not None:
            params['minExpansionDist'] = float(actual_min)
        if actual_max is not None:
            params['maxExpansionDist'] = float(actual_max)

        battery_summaries = []
        preview_paths = []
        preview_batteries = []
        for battery_index in range(batteries):
            export_data = _build_real_path_battery_export(
                designer=designer,
                params=params,
                center_str=center,
                battery_index=battery_index,
            ...
            preview_paths.append(export_data['previewPath'])
            preview_batteries.append({
                'batteryIndex': battery_index + 1,
                'waypoints': export_data['previewWaypoints'],
                'telemetry': telemetry,
            })
        ...
        response_body = {
            'optimizedParams': {
                **optimized_params,
                ...
                'formToTerrain': form_to_terrain,
                'actualMinExpansionDist': actual_min,
                'actualMaxExpansionDist': actual_max,
            },
            'optimizationInfo': {
            ...
            'previewBatteries': preview_batteries,
        ...
        return {
            'statusCode': 500,
            'headers': cors_headers,
            'body': json.dumps({'error': f'Spin path optimization failed: {str(e)}'})
        }


def handle_spin_path_export_battery(designer, body, battery_id, cors_headers):
    try:
        if not battery_id:
            return {
                'statusCode': 400,
        ...
        battery_index = int(battery_id) - 1
        slices = int(body.get('slices', 1))
        center = body.get('center', '')
        if not center:
            return {
                'statusCode': 400,
        ...
        params = {
            'slices': slices,
            'N': int(body.get('N', 8)),
            'r0': float(body.get('r0', 150.0)),
            'rHold': float(body.get('rHold', 1595.0)),
        }
        min_exp, max_exp = _parse_expansion_inputs(body)
        ...
        return {
            ...
            'body': json.dumps({
                ...
                'previewPath': export_data['previewPath'],
                'previewWaypoints': export_data['previewWaypoints'],
            }),
        }
    except TerrainElevationUnavailableError as e:
        return {
        ...
        return {
            'statusCode': 500,
            'headers': cors_headers,
            'body': json.dumps({'error': f'Spin path battery export failed: {str(e)}'})
        }
