...
def _bundle_skybox_asset(bundle_dir: Path) -> str | None:
    generated_skybox = bundle_dir / "background_skybox.webp"
    if generated_skybox.exists():
        skybox_dir = bundle_dir / "skybox"
        skybox_dir.mkdir(parents=True, exist_ok=True)
        destination = skybox_dir / generated_skybox.name
        shutil.copy2(generated_skybox, destination)
        logger.info("Bundled generated Splatfacto-W skybox %s into SuperSplat bundle", generated_skybox.name)
        return f"skybox/{generated_skybox.name}"
...
class PlayCanvasSOGSCompressor:
    ...
    def _create_supersplat_bundle(self, results: Dict[str, Any], supporting_files: Iterable[Path] = ()) -> list[str]:
        ...
        copied_supporting_files = self._copy_supporting_files(supporting_files, bundle_dir)
        ...
        return copied_supporting_files
    ...
    def process_job(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="sogs-work-") as temp_dir:
            ...
            copied_supporting_files = self._create_supersplat_bundle(results, source.supporting_files)
