...
SFM_AUTHORITY_VERSION = "seam_graph_sim3_v1"
...
def stable_json_sha256(payload: Mapping[str, Any] | None) -> str | None:
    if payload is None:
        return None
...
def _leaf_to_tile_id(value: Any) -> str:
    raw_value = str(value).strip()
    try:
        return f"tile_{int(raw_value):02d}"
    except (TypeError, ValueError):
        if raw_value.startswith("tile_"):
            return raw_value
        return raw_value
...
def normalize_sfm_seam_graph(
    *,
    reducer_metadata: Mapping[str, Any] | None = None,
    seam_merge_report: Mapping[str, Any] | None = None,
) -> dict[str, Any] | None:
    """Normalize SfM reducer seam proof into a downstream 3DGS authority contract."""

    reducer = reducer_metadata if isinstance(reducer_metadata, Mapping) else {}
    seam_report = seam_merge_report if isinstance(seam_merge_report, Mapping) else {}
    if not reducer and not seam_report:
        return None

    source = seam_report or reducer
    accepted_source = source.get("accepted_merge_tree") or reducer.get("accepted_merge_tree") or []
    accepted_edges: list[dict[str, Any]] = []
    for edge_index, edge in enumerate(accepted_source):
        if not isinstance(edge, Mapping):
            continue
        leaf_a = edge.get("leaf_a", edge.get("source_leaf", edge.get("a")))
        leaf_b = edge.get("leaf_b", edge.get("target_leaf", edge.get("b")))
        tile_a = _leaf_to_tile_id(leaf_a)
        tile_b = _leaf_to_tile_id(leaf_b)
        if not tile_a or not tile_b or tile_a == tile_b:
            continue
        tile_ids = sorted([tile_a, tile_b])
        accepted_edges.append(
            {
                "seam_id": edge.get("seam_id") or f"{tile_ids[0]}__{tile_ids[1]}",
                "leaf_ids": [leaf_a, leaf_b],
                "tile_ids": tile_ids,
                "confidence": float(edge.get("confidence", 0.0) or 0.0),
                "shared_registered_images": _as_positive_int(edge.get("shared_registered_images")),
                "strict_decision": str(edge.get("strict_decision", edge.get("decision", "accept"))),
                "blockers": list(edge.get("blockers", [])) if isinstance(edge.get("blockers", []), list) else [],
                "source_edge_index": edge_index,
            }
        )

    blockers = []
    for payload in (reducer, seam_report):
        raw_blockers = payload.get("promotion_blockers", [])
        if isinstance(raw_blockers, list):
            blockers.extend(raw_blockers)
    cycle_consistency = source.get("cycle_consistency") or reducer.get("cycle_consistency") or {}
    cycle_status = str(cycle_consistency.get("status", "")).lower() if isinstance(cycle_consistency, Mapping) else ""
    cycle_failures = cycle_consistency.get("failures", []) if isinstance(cycle_consistency, Mapping) else []
    decision = str(source.get("decision", reducer.get("decision", ""))).lower()
    leaf_count = _as_positive_int(source.get("leaf_count", reducer.get("leaf_count")))
    status = "pass"
    if blockers:
        status = "fail"
    if decision and decision not in {"pass", "passed", "promote", "production_ready_sfm_only"}:
        status = "fail"
    if cycle_status and cycle_status not in {"pass", "passed", "ok"}:
        status = "fail"
    if isinstance(cycle_failures, list) and cycle_failures:
        status = "fail"
    if leaf_count > 1 and not accepted_edges:
        status = "fail"

    return {
        "version": SFM_AUTHORITY_VERSION,
        "authority": "sfm_reducer",
        "status": status,
        "decision": source.get("decision", reducer.get("decision")),
        "merge_strategy": source.get("strategy") or reducer.get("merge_strategy") or SFM_AUTHORITY_VERSION,
        "leaf_count": leaf_count,
        "accepted_edges": accepted_edges,
        "accepted_edge_count": len(accepted_edges),
        "rejected_edge_count": _as_positive_int(source.get("rejected_edge_count", reducer.get("rejected_edge_count"))),
        "cycle_consistency": cycle_consistency,
        "post_merge_jurisdiction_culling": source.get("post_merge_jurisdiction_culling")
        or reducer.get("post_merge_jurisdiction_culling")
        or {},
        "post_merge_global_double_surface_culling": source.get("post_merge_global_double_surface_culling")
        or reducer.get("post_merge_global_double_surface_culling")
        or {},
        "promotion_blockers": blockers,
        "source_hashes": {
            "reducer_metadata_sha256": stable_json_sha256(reducer_metadata),
            "seam_merge_report_sha256": stable_json_sha256(seam_merge_report),
        },
    }


def validate_sfm_authority_contract(
    seam_graph: Mapping[str, Any] | None,
    *,
    tile_ids: Sequence[str],
    require_sfm_authority: bool = False,
) -> dict[str, Any]:
    normalized_tile_ids = {str(tile_id) for tile_id in tile_ids if str(tile_id).strip()}
    errors: list[str] = []
    warnings: list[str] = []
    if seam_graph is None:
        if require_sfm_authority:
            errors.append("missing_sfm_seam_graph")
        return {"status": "fail" if errors else "not_available", "errors": errors, "warnings": warnings}

    if seam_graph.get("version") != SFM_AUTHORITY_VERSION:
        errors.append("unsupported_sfm_seam_graph_version")
    if seam_graph.get("status") != "pass":
        errors.append("sfm_seam_graph_not_pass")

    accepted_edges = [edge for edge in seam_graph.get("accepted_edges", []) if isinstance(edge, Mapping)]
    if len(normalized_tile_ids) > 1 and not accepted_edges:
        errors.append("sfm_seam_graph_has_no_accepted_edges")

    edge_tile_ids: set[str] = set()
    missing_edge_support = 0
    for edge in accepted_edges:
        pair = [str(tile_id) for tile_id in edge.get("tile_ids", []) if str(tile_id).strip()]
        edge_tile_ids.update(pair)
        if _as_positive_int(edge.get("shared_registered_images")) <= 0:
            missing_edge_support += 1
    if missing_edge_support:
        errors.append("accepted_sfm_seam_missing_shared_registered_images")

    missing_tiles = sorted(edge_tile_ids - normalized_tile_ids)
    isolated_tiles = sorted(normalized_tile_ids - edge_tile_ids) if len(normalized_tile_ids) > 1 else []
    if missing_tiles:
        warnings.append("sfm_seam_graph_references_tiles_absent_from_manifest")
    if isolated_tiles:
        errors.append("manifest_tiles_isolated_from_sfm_seam_graph")
...
def _seam_edge_by_tile_pair(seam_graph: Mapping[str, Any] | None) -> dict[tuple[str, str], dict[str, Any]]:
    if not isinstance(seam_graph, Mapping):
        return {}
    edge_by_pair: dict[tuple[str, str], dict[str, Any]] = {}
    for edge in seam_graph.get("accepted_edges", []):
        if not isinstance(edge, Mapping):
            continue
        tile_ids = [str(tile_id) for tile_id in edge.get("tile_ids", []) if str(tile_id).strip()]
        if len(tile_ids) != 2:
            continue
        edge_by_pair[tuple(sorted(tile_ids))] = dict(edge)
    return edge_by_pair
...
def rank_candidate_tile_pairs(
    ...
    *,
    ...
    sfm_seam_graph: Mapping[str, Any] | None = None,
    ...
    seam_edge_by_pair = _seam_edge_by_tile_pair(sfm_seam_graph or tile_manifest.get("sfm_seam_graph"))
    pair_keys: set[tuple[str, str]] = {
        pair for pair in seam_edge_by_pair if pair[0] in tile_by_id and pair[1] in tile_by_id
    }
    for tile in tiles:
        tile_id = str(tile.get("tile_id", "")).strip()
        if not tile_id:
    ...
    for first_id, second_id in sorted(pair_keys):
        ...
        seam_edge = seam_edge_by_pair.get(tuple(sorted((first_id, second_id))), {})
        ...
        if boundary_support["poor_boundary_support"] and not seam_edge:
        ...
        sfm_shared_registered_images = _as_positive_int(seam_edge.get("shared_registered_images"))
        sfm_confidence = float(seam_edge.get("confidence", 0.0) or 0.0)
        if seam_edge and sfm_shared_registered_images <= 0:
            ineligible_reasons.append("sfm_accepted_seam_missing_registered_support")
        ...
        score = (
            ...
            + min(sfm_confidence, 300.0) * 0.25
            + sfm_shared_registered_images * 0.5
        ...
        ranked_pairs.append(
            {
                ...
                "sfm_authority_edge": bool(seam_edge),
                "sfm_seam_id": seam_edge.get("seam_id"),
                "sfm_seam_confidence": sfm_confidence,
                "sfm_shared_registered_images": sfm_shared_registered_images,
...
def enrich_tile_manifest_support_metadata(
    ...
    *,
    ...
    if sfm_seam_graph is not None:
        manifest["sfm_seam_graph"] = dict(sfm_seam_graph)
    seam_graph = manifest.get("sfm_seam_graph") if isinstance(manifest.get("sfm_seam_graph"), Mapping) else None
    seam_edge_by_pair = _seam_edge_by_tile_pair(seam_graph)
    ...
    for tile in tiles:
        ...
        for neighbor_id in ordered_unique(tile.get("neighbor_tile_ids", [])):
            ...
            if neighbor is None:
                continue
            seam_edge = seam_edge_by_pair.get(tuple(sorted((tile_id, neighbor_id))), {})
            ...
            overlap_stats[neighbor_id] = {
                ...
                "sfm_authority_edge": bool(seam_edge),
                "sfm_seam_id": seam_edge.get("seam_id"),
                "sfm_seam_confidence": float(seam_edge.get("confidence", 0.0) or 0.0),
                "sfm_shared_registered_images": _as_positive_int(seam_edge.get("shared_registered_images")),
            }
    ...
    authority_validation = validate_sfm_authority_contract(
        seam_graph,
        tile_ids=[str(tile.get("tile_id")) for tile in tiles if tile.get("tile_id")],
        require_sfm_authority=require_sfm_authority,
    )
    if authority_validation["status"] == "fail" and require_sfm_authority:
        raise ValueError(f"SfM seam authority contract failed: {authority_validation['errors']}")
    manifest["sfm_authority_validation"] = authority_validation
    candidate_pairs = rank_candidate_tile_pairs(manifest, view_buckets, sfm_seam_graph=seam_graph)
    manifest["support_statistics"] = {
        ...
        "sfm_authority_status": authority_validation["status"],
    }
...
def synthesize_tiled_inputs_from_chunk_planner(
    ...
    *,
    ...
    sfm_seam_graph = normalize_sfm_seam_graph(
        reducer_metadata=reducer_metadata,
        seam_merge_report=seam_merge_report,
    )
    sfm_neighbor_ids: dict[str, list[str]] = {}
    for edge in (sfm_seam_graph or {}).get("accepted_edges", []):
        if not isinstance(edge, Mapping):
            continue
        tile_ids = [str(tile_id) for tile_id in edge.get("tile_ids", []) if str(tile_id).strip()]
        if len(tile_ids) != 2:
            continue
        first_tile_id, second_tile_id = sorted(tile_ids)
        sfm_neighbor_ids.setdefault(first_tile_id, []).append(second_tile_id)
        sfm_neighbor_ids.setdefault(second_tile_id, []).append(first_tile_id)
    ...
    for index, chunk in enumerate(chunks):
        ...
        neighbor_tile_ids: list[str] = ordered_unique(sfm_neighbor_ids.get(tile_id, []))
    ...
    manifest = {
        ...
        "tiles": tiles,
        "sfm_seam_graph": sfm_seam_graph,
        "parent_hashes": {
            "chunk_planner_manifest_sha256": stable_json_sha256(chunk_planner_manifest),
            "sfm_metadata_sha256": stable_json_sha256(sfm_metadata),
            "reducer_metadata_sha256": stable_json_sha256(reducer_metadata),
            "seam_merge_report_sha256": stable_json_sha256(seam_merge_report),
        },
        "manifest_resolution": {
            ...
            "sfm_authority_available": sfm_seam_graph is not None,
        },
    }
...
def resolve_tiled_input_manifests(
    *,
    ...
    if isinstance(tile_manifest_payload, Mapping) and isinstance(view_bucket_payload, Mapping):
        ...
        if sfm_seam_graph is not None:
            manifest["sfm_seam_graph"] = sfm_seam_graph
        manifest.setdefault("parent_hashes", {})
        if isinstance(manifest["parent_hashes"], dict):
            manifest["parent_hashes"].update(
                {
                    "chunk_planner_manifest_sha256": stable_json_sha256(chunk_planner_manifest),
                    "sfm_metadata_sha256": stable_json_sha256(sfm_metadata),
                    "reducer_metadata_sha256": stable_json_sha256(reducer_metadata),
        ...
        resolution = {
            ...
            "sfm_authority_available": isinstance(manifest.get("sfm_seam_graph"), Mapping),
            "sfm_authority_status": manifest.get("sfm_authority_validation", {}).get("status"),
