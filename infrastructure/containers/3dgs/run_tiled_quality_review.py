...
def resolve_review_manifest_inputs(
    *,
    ...
    reducer_metadata_source = review_input_dir / "reducer_metadata.json"
    seam_merge_report_source = review_input_dir / "seam_merge_report.json"
    ...
    reducer_metadata_payload = load_json(reducer_metadata_source) if reducer_metadata_source.exists() else None
    seam_merge_report_payload = load_json(seam_merge_report_source) if seam_merge_report_source.exists() else None
    ...
    tile_manifest, view_buckets, manifest_resolution = resolve_tiled_input_manifests(
        ...
        require_sfm_authority=str(os.environ.get("REQUIRE_SFM_AUTHORITY", "")).lower() in {"1", "true", "yes", "on"},
...
def exposure_normalize_prediction(prediction: np.ndarray, target: np.ndarray) -> np.ndarray:
    prediction = np.clip(prediction.astype(np.float32), 0.0, 1.0)
    target = np.clip(target.astype(np.float32), 0.0, 1.0)
    pred_mean = np.mean(prediction, axis=(0, 1), keepdims=True)
    pred_std = np.std(prediction, axis=(0, 1), keepdims=True)
    target_mean = np.mean(target, axis=(0, 1), keepdims=True)
    target_std = np.std(target, axis=(0, 1), keepdims=True)
    normalized = (prediction - pred_mean) * (target_std / np.maximum(pred_std, 1e-4)) + target_mean
    return np.clip(normalized, 0.0, 1.0)


def edge_alignment_stats(reference: np.ndarray, prediction: np.ndarray) -> dict[str, float]:
    ...
    prediction_edges = edge_strength(prediction)
    reference_threshold = float(np.percentile(reference_edges, 85))
    prediction_threshold = float(np.percentile(prediction_edges, 85))
    reference_mask = reference_edges >= max(reference_threshold, 1e-4)
    prediction_mask = prediction_edges >= max(prediction_threshold, 1e-4)
    intersection = np.logical_and(reference_mask, prediction_mask)
    precision = float(np.sum(intersection) / max(1, np.sum(prediction_mask)))
    recall = float(np.sum(intersection) / max(1, np.sum(reference_mask)))
    f_score = float((2.0 * precision * recall) / max(precision + recall, 1e-8))
    return {
        "edge_precision": precision,
        "edge_recall": recall,
        "edge_f_score": f_score,
        "reference_edge_density": float(np.mean(reference_mask)),
        "prediction_edge_density": float(np.mean(prediction_mask)),
    }
    ...
    try:
        ...
        for bucket_name, bucket_label_value in DEFAULT_BUCKET_ORDER:
            for image_name in review_images_by_bucket.get(bucket_name, []):
                ...
                exposure_normalized_render = exposure_normalize_prediction(merged_final, reference_image)
                metrics_exposure_normalized = compute_metrics(
                    exposure_normalized_render,
                    reference_image,
                    lpips_model=lpips_model,
                    device=device,
                )
                ...
                view_entry: dict[str, Any] = {
                    ...
                    "metrics": metrics,
                    "metrics_exposure_normalized": metrics_exposure_normalized,
