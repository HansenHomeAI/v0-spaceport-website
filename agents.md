...
4. **Iterate**: apply the smallest fix, redeploy, watch the live preview build/logs, and rerun the baseline until everything passes. One pass is never enough—stay in the loop and repeat (fix → push → deploy → validate → test) until the previewed experience matches the spec with no regressions. Log every loop in `logs/agent-loop.log` and do not stop while there are open findings or unverified behaviours.
